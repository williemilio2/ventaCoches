...
useEffect(() => {
  const handleScroll = () => {
    ...
    setShowButton(rect.top < window.innerHeight - 1000 && rect.bottom > 500)
  }
  ...
  return (
    <section id="catalogo" className="py-20 bg-background relative">
      ...
      {showButton && (
        <a
          href="#sobre-nosotros"
          className="fixed bottom-6 right-6 md:bottom-25 md:right-6 bottom-22 right-4 flex items-center justify-center w-14 h-14 rounded-full bg-primary text-white shadow-lg hover:scale-110 hover:bg-primary/90 transition-all group z-50"
        >
          <ArrowDown className="w-7 h-7 group-hover:translate-y-0.5 transition-transform" />
        </a>
      )}
